...
import { autoAssignIssueRoundRobin, createIssue, deleteIssue, getMe, listExperts, listIssues, updateMe, type AiResult, type AuthUser, type Issue } from "@/services";
            ...
            async function submit(e:React.FormEvent<HTMLFormElement>){e.preventDefault(); 
                ...
                const timer=window.setInterval(()=>{idx=Math.min(phases.length-1,idx+1); 
                    ...
                    try{ 
                        ...
                        try {
                            const experts = await listExperts({ available: true });
                            await autoAssignIssueRoundRobin(created.id, experts);
                    ...
                    icon={<CalendarDays size={20}/>}/><StatCard label="AI ready" value="LLM" icon={<Brain size={20}/>}/></div><div className="mt-6 grid gap-6 lg:grid-cols-[.95fr_1.05fr]"><Card className="p-5"><div className="mb-5 flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-linear-to-br from-teal-200 via-sky-200 to-violet-200 text-slate-950"><PlusCircle/></div><div><h2 className="font-black">Create new issue</h2><p className="text-sm text-slate-500">Backend AI will classify category and urgency.</p></div></div><form onSubmit={submit} className="space-y-4"><input className="input" placeholder="Title" value={form.title} 
