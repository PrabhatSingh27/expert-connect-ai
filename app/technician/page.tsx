...
import { acceptAssignedIssue, createSlot, deleteSlot, getMyAvailabilities, getMyExpertProfile, listAssignedExpertIssues, updateExpertProfile, updateIssueStatus, updateSlot, type AvailabilitySlot, type Expert, type Issue } from "@/services";
...
export default function TechnicianPage() {
  ...
  return <DashboardShell title="Expert dashboard" subtitle="Manage profile, weekly availability, and assigned jobs." action={<button onClick={refresh} className="inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm font-bold"><RefreshCw size={15}/>Refresh</button>}>
    <div className="grid gap-4 md:grid-cols-4"><StatCard label="Assigned jobs" value={jobs.length} icon={<Briefcase size={20}/>}/><StatCard label="Active jobs" value={activeJobs.length} icon={<BadgeCheck size={20}/>}/><StatCard label="Availability slots" value={slots.length} icon={<CalendarPlus size={20}/>}/><StatCard label="Rating" value={profile?.rating ?? "-"} icon={<Star size={20}/>}/></div>
